...
namespace AgWebTool
{
    public partial class Form1 : Form
    {
        ...
        void LoadingShow()
        {
            new Thread(() => new Form6().ShowDialog()).Start();
        }

        void LoadingClose()
        {
            Form6 Loading = new Form6();
            Loading = (Form6)Application.OpenForms["Form6"];
            Loading.Invoke(new ThreadStart(delegate { Loading.Close(); }));
        }
                ...
                if (listBox4.SelectedItem != null)
                {
                    ...
                    else
                    {
                        if (!bHistory)
                        {
                            ...
                            newLines.Add("<ID>" + nSelected + "</ID><ADDRESS>" + historyaddr + "</ADDRESS><NAME>" + SearchDataClassList[nSelected].label + "</NAME><TIMESTAMP>" + time + "</TIMESTAMP>");
